...
初始化时按以下模板在项目根目录生成 `AGENTS.md`，结合项目实际填写。它是共享主干文件：只在 main 分支修改，改后立即推送并 `git merge main` 同步到各 worktree。
...
项目约定变化（记忆位置、Git/worktree 规则、_temp 规范、shell 约定）后必须同步更新本文件，保持与实际约定一致。
